...
public class NoraGatewayForAndroidApp extends Application {
	...
	public void onCreate() {
		...
		usbMonitor = new USBMonitor(this, usbMonitorOnDeviceConnectListener);
		usbMonitor.register();
		...
		if(usbDevices != null && usbDevices.size() >= 1){
			for(UsbDevice usbDevice : usbDevices){
				if(
						...
						!usbMonitor.hasPermission(usbDevice)
...
//		startService(new Intent(getBaseContext(), OnClearFromRecentService.class));
	...
	private final USBMonitor.OnDeviceConnectListener usbMonitorOnDeviceConnectListener =
