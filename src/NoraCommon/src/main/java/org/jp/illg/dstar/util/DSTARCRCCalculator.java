...
public class DSTARCRCCalculator {

	private static final int[] crcTabCCITT =
	{
		0x0000, 0x1189, 0x2312, 0x329b, 0x4624,
	...
	public int getResultCRC() {
		return getResultCRC(crcTemp);
	}

	public void updateCRC(byte c) {
		crcTemp = updateCRC(crcTemp, c);
	}

	public static int calcCRCRange(byte[] data, int start, int end) {
		if(
			data == null ||
			start < 0 ||
			end < 0 ||
		...
		return calcCRC(data, start, (end - start + 1));
	}

	public static int calcCRC(byte[] data, int length) {
		if(data == null || length < 0) {new IllegalArgumentException();}

		return calcCRC(data, 0, length);
	}

	public static int calcCRC(byte[] data) {
		if(data == null) {new IllegalArgumentException();}

		return calcCRC(data, 0, data.length);
	}

	public static int calcCRC(byte[] data, int startOffset, int length) {
		if(
			data == null ||
			startOffset < 0 ||
			length < 0
		...
		for (n = startOffset; n < (startOffset + length) && n < data.length; n++)
			temp = updateCRC(temp, data[n]);

		return getResultCRC(temp);
	}

	private static int getResultCRC(int crc) {
		return ~crc & 0xFFFF;
	}

	private static int updateCRC(int crc, byte c) {
		int tmp, short_c;
